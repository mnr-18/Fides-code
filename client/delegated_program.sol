...
contract Matrix {
	...
	uint256 [6][6] public output;
	
	function compute (uint256[6][6] memory matA, uint256[6][6] memory matB) public {
		output = mat3Mult(matA,matB);
	}
	
	function mat3Mult(uint256[6][6] memory mat1, uint256[6][6] memory mat2) private pure returns (uint[6][6] memory) { 
        uint r1 = mat1.length; // rows of mat1
        uint c1 = mat1[0].length; // columns of mat1
        uint c2 = mat2[0].length; // columns of mat2

        uint256[6][6] memory result; 
